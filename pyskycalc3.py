...
from thorskyutil import flmoon, lunskybright, thorconsts
        ...
        # 2008 Jan -- let's rewrite this using xpa access points and a more sophisticated everything.
                ...
                if not self.ds9_is_open() :   # there is no ds9 up ...
                        ...
                        while (not self.ds9_is_open()) and ntries < 10:
                                sleep(0.5)  # have to wait for process to start before commanding it ... this is a guess
                                ntries = ntries + 1     
...
class Quitter(Frame):                          # subclass our GUI
    def __init__(self, parent=None):           # constructor method
      ...
      #  widget = Button(self, text='Quit', command=self.quit, bg="#ffaa75")
        widget = Button(self, text='Quit', command=self.quit, width=3, bg="#ffaa75")
...
#       time2 = var2.get()
        ...
        try : 
            second = int(x[2])
            microsecond = int((float(x[2]) - second) * 1000000.)
        except :
            second = 0
            microsecond = 0
        localfl = var3
...
Button(topbuttonpan, text = 'Refresh\n output',
   command = (lambda v=invars,o=outvars,h=havars,a=almvars,c=coovars,
      p=planetvars,b=outboxes,hb=haboxes,pb=planetboxes,sd=skdisp, ad=airmdisp: 
        circumstances(v,o,h,a,c,p,b,hb,pb,sd,ad)),width=5).pack(side=LEFT)
Button(topbuttonpan, text = 'Step\n time',
   command = (lambda v=invars,o=outvars,h=havars,a=almvars,c=coovars,
      p=planetvars,b=outboxes,hb=haboxes,pb=planetboxes,sd=skdisp,ad=airmdisp: 
        steptime(v,o,h,a,c,p,b,hb,pb,sd,ad,True)),width=3).pack(side=LEFT)
Button(topbuttonpan, text = 'Set to\n now',
   command = (lambda v=invars,o=outvars,h=havars,a=almvars,c=coovars,
      p=planetvars,b=outboxes,hb=haboxes,pb=planetboxes,sd=skdisp,ad=airmdisp: 
        set_to_now(v,o,h,a,c,p,b,hb,pb,sd,ad)),width=4).pack(side=LEFT)

Button(topbuttonpan, text = 'UT <->\n local',
   command = (lambda v=invars,o=outvars,h=havars,a=almvars,c=coovars,
     p=planetvars,b=outboxes,hb=haboxes,pb=planetboxes,sd=skdisp,ad=airmdisp: 
        convert_time(v,o,h,a,c,p,b,hb,pb,sd,ad)),width=4).pack(side=LEFT,fill=Y)
...
Button(topbuttonpan, text = 'Site\n menu',
   command = (lambda w = sitewin : win_raise(w)),width=3).pack(side=LEFT)

Button(topbuttonpan, text = 'Sky\nDisplay',
   command = (lambda sd = skdisp, o=obs, obj=objs2000 : skydisplay_toggle(sd,o,obj)),width=4).pack(side=LEFT,fill=Y)
Button(topbuttonpan, text = 'Airmass\nDisplay',
   command = (lambda ad = airmdisp, o=obs : airmdisplay_toggle(ad,o)),width=4).pack(side=LEFT,fill=Y)
Button(topbuttonpan, text = 'Nightly\n Almanac',
   command = (lambda w = almwin: win_raise(w)),width=5).pack(side=LEFT)

bottombuttonpan = Frame(bottompan)
bottombuttonpan.pack(side = TOP)
Button(bottombuttonpan, text = 'Alt.\nCoords',
   command = (lambda w = coowin : win_raise(w)),width=4).pack(side=LEFT)
...
Button(bottombuttonpan, text = 'Text\nTables',
   command = (lambda v = invars : text_table_win(v)),width=4).pack(side=LEFT)

Button(bottombuttonpan, text = 'DSS\n window',
   command = (lambda i = imwin : win_raise(i)),width=4).pack(side=LEFT,anchor=W,fill=Y)
